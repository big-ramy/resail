...
function setUserLanguage(lang) {
    // 1. تحقق إذا كانت اللغة الجديدة مختلفة عن الحالية
    if (lang === currentLang) {
        // لا تفعل شيئًا إذا لم تتغير اللغة
        return;
    }

    // ⭐ التعديل الأساسي: تم حذف السطر الذي كان يقوم بمسح البيانات من هنا
    // localStorage.removeItem('resailCvData_' + currentLang);

    // 3. تحديث اللغة الحالية
    currentLang = lang;

    // 4. تحديد اختيار اللغة الجديد في ذاكرة الجلسة (sessionStorage)
    sessionStorage.setItem('userLang', lang);

    // 5. إعادة تحميل الصفحة باللغة الجديدة
    window.location.href = lang === 'ar' ? 'index.html' : 'en.html';
}

/**
 * يجمع كل بيانات نموذج السيرة الذاتية ويحفظها في localStorage
 * باستخدام مفتاح خاص باللغة الحالية (مثال: 'resailCvData_ar').
 */
function saveCvDataToLocalStorage() {
    ...
    try {
        // ⭐ التغيير الرئيسي هنا: إضافة currentLang إلى المفتاح
...
/**
 * يفحص localStorage عن بيانات محفوظة للغة الحالية، وإذا وجدت، يقوم بتعبئة النموذج بها.
 * @returns {boolean} - يُرجع 'true' إذا تم العثور على بيانات وتحميلها، و'false' بخلاف ذلك.
 */
function loadCvDataFromLocalStorage() {
    try {
        // ⭐ التغيير الرئيسي هنا: قراءة المفتاح الخاص باللغة الحالية
        const savedDataJSON = localStorage.getItem('resailCvData_' + currentLang);
        if (!savedDataJSON) {
            return false; // لا توجد بيانات محفوظة لهذه اللغة
        }

        const savedData = JSON.parse(savedDataJSON);

        // ... (بقية الكود في هذه الدالة يبقى كما هو دون تغيير) ...
...
document.addEventListener('DOMContentLoaded', () => {
    // ⭐ تم تصحيح طريقة العثور على العناصر هنا
    loadingOverlayGlobal = document.getElementById('loading-overlay');
    loadingTextGlobal = document.querySelector('#loading-overlay p'); // استخدمنا محدد أبسط وأكثر دقة

    // بقية الكود في هذا الجزء يبقى كما هو
...
function showPage(pageId) {
    ...
    if (targetPage) {
        ...
        // ⭐ التعديل الأساسي: منطق ذكي لتحميل البيانات أو عرض البيانات التجريبية
        if (pageId === 'cv-data-entry-page') {
            // أولاً، حاول تحميل البيانات المحفوظة للمستخدم
            const dataWasLoaded = loadCvDataFromLocalStorage();

            // إذا لم يتم العثور على أي بيانات محفوظة لهذه اللغة، قم بعرض البيانات التجريبية
            if (!dataWasLoaded) {
                populateWithTestData();
            }
            
            // في كل الأحوال، قم بتحديث المعاينة وشريط التقدم
...
function getSkillsData() {
    ...
    document.querySelectorAll('#skills-input .skill-entry').forEach(entry => {
        ...
        // ⭐ التعديل الرئيسي هنا: نحفظ المهارة طالما أن حقل الاسم ليس فارغًا
        if (name) {
...
/**
 * =========================================================================
 * == دالة generatePdfFromNode النهائية (بعد فصل ملفات CSS) ==
 * =========================================================================
 * تقوم بجمع محتوى ملفات CSS الخارجية ودمجها مع HTML قبل إرسالها للسيرفر.
...
async function generatePdfFromNode(isPaid) {
    ...
    try {
        ...
        // ⭐ الخطوة 1: جلب محتوى ملفات CSS الخارجية كنصوص
        ...
        // --- بقية الدالة تبقى كما هي تقريبًا ---
        ...
        // ⭐ الخطوة 2: حقن كود CSS المجمع داخل الـ HTML الذي سيتم إرساله
        const fullPageHtml = `
            <!DOCTYPE html>
