...
async function getCVDocumentDefinition(addWatermark) {
    ...
    if (emailVal) contactItems.push({ text: (isArabic ? '\u2709  ' : '\u2709  ') + emailVal, style: 'text' }); // أيقونة مظروف
    if (phone) contactItems.push({ text: (isArabic ? '\u260E  ' : '\u260E  ') + phone, style: 'text' }); // أيقونة هاتف
    if (website) contactItems.push({ text: (isArabic ? '\u1F310  ' : '\u1F310  ') + website, style: 'text' }); // أيقونة كرة أرضية
    ...
    if (filledSkills.length > 0) {
        ...
        skillsContent.push({
            ul: filledSkills.map(skill => ({
                text: skill,
                style: 'listItem',
                // لإزالة النقطة، نستخدم listType: 'none' في خصائص ul
                // أو يمكن إضافة بادئة مخصصة هنا مثل '- '
            })),
            margin: [0, 0, 0, 5],
            listType: 'none', // إزالة النقاط الافتراضية
